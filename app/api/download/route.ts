...
export async function POST(request: NextRequest) {
  try {
    ...
    // Use txHash as orderId if orderId not provided
    const id = orderId || txHash
    ...
    try {
      // Try to query database if it exists
      ...
      if (existingTickets.length > 0) {
        ...
        const newTickets = await sql`
          ...
          VALUES (
            ${ticketId}, ${id}, ${order.storage_key}, ${expiresAt}, 3, 0
          )
          RETURNING *
        `
        ticket = newTickets[0]
      ...
      await sql`
        ...
        VALUES (
          ${ticket.ticket_id}, ${id}, ${buyerAddress.toLowerCase()}, ${order.product_id}, ${ipAddress}, ${userAgent}
        )
      `
      ...
      if (dbError.message?.includes("relation") && dbError.message?.includes("does not exist")) {
        console.log("[v0] Database not initialized, returning mock download with one-time link")

        const mockTicketId = `ticket-${crypto.randomBytes(8).toString("hex")}`
        const expiresAt = Date.now() + 10 * 60 * 1000 // 10 minutes
        const signature = crypto.createHash("sha256").update(`${mockTicketId}:${expiresAt}`).digest("hex")
        const downloadUrl = `/api/download/file?ticket=${mockTicketId}&expires=${expiresAt}&sig=${signature}&maxUses=3`
        ...
        return NextResponse.json({
          ...
          note: "Mock download link - database not initialized",
        })
      }

      console.error("[v0] Database error in download:", dbError.message)
      return NextResponse.json(
        {
          success: true,
          downloadUrl: "/api/download/file?mock=true",
          expiresIn: 600,
          remainingDownloads: 2,
          checksum: "mock-checksum",
        },
